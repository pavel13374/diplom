...
def main():
    ...
    print("=" * 76)
    # Вывод формулируется ПО ДАННЫМ, а не зашит в текст. Раньше здесь стояла
    # фраза «решающий вклад даёт поведенческий слой» независимо от чисел —
    # и после исправления фона она перестала соответствовать результату.
    drop = noisy["recall"] - stealth["recall"]
    gain = share_behavior(stealth) - share_behavior(noisy)
    print(f"  ВЫВОД: под stealthy recall правил падает на {drop*100:.0f} п.п. — "
          f"уклонение по содержимому")
    print(f"  действительно ослепляет сигнатурный слой.")
    if gain > 0.10:
        print(f"  Поведенческий слой компенсирует потерю: доля пойманных ТОЛЬКО им")
        print(f"  растёт на {gain*100:.0f} п.п. Это то, что регуляркой не покрыть.")
    else:
        print(f"  Поведенческий слой компенсирует лишь малую часть "
              f"(+{gain*100:.0f} п.п. эпизодов,")
        print("  пойманных только им): UEBA работает под жёстким бюджетом тревог и")
        print("  один эпизод из десятка вытягивает, но не заменяет правила. Основной")
        print("  прирост под уклонением даёт слой ML — см. research/experiments.py.")
