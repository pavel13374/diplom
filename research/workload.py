...
FILE_KINDS = [
    ...
    ("ci/{b}.gitlab-ci.yml",     0.03, 4.20, 0.30, 6.0),
]
...
def benign_push_features(rnd):
    ...
    Признаки берутся из распределений, а часть файлов содержит benign-двойники
    секретов (SHA-40, UUID, base64) — чтобы FP-бюджет был честным.
    """
...
def build_workload(evasion="noisy", seed=42, days=12, per_day=140, campaigns=None):
    ...
    # Кампании раскладываем по РАЗНЫМ дням, а не по часам одного дня:
    # раньше здесь было datetime(..., 19 + i, ...) и на пятой кампании
    # (а их 12) падало с ValueError: hour must be in 0..23.
    for i, key in enumerate(keys):
        day = start + timedelta(days=2 + (i % max(1, days - 3)),
                                hours=(9 + (i * 3) % 12) - start.hour)
