...
Что проверяем:
  ...
  5. Движение ограничено списком. Полный запрет @keyframes был верен
     наполовину: анимация ПО ХОВЕРУ на плотном интерфейсе читается как
     подтормаживание, но появление нового слоя без движения читается как
     подмена картинки. Поэтому проверяется не отсутствие анимации, а её
     границы: пять именованных анимаций, ни одной длиннее 300 мс, ни одной
     на :hover, движутся только transform и opacity.
...
def main():
    ...
    # Градиент как ДЕКОР запрещён. Разрешены ровно четыре служебных:
    #   1 — индикатор уровня high (полузаливка точки бейджа);
    #   2,3 — разлив света на полотне, по одному на тему: он даёт экрану
    #         источник света и отделяет сайдбар без второй заливки;
    #   4 — бегущий блик скелета загрузки: он сообщает, что данные идут.
    # Пятый градиент означает, что цветом снова начали украшать.
    grads = re.findall(r"background(?:-image)?\s*:[^;}]*(?:linear|radial)-gradient\(", css_nc)
    check(len(grads) <= 4,
          "в CSS не больше четырёх служебных градиентов",
          "найдено: %d" % len(grads))
    ...
    # 5. движение
    # 5а. Список анимаций закрыт. Каждая заведена под конкретную задачу
    #     появления; шестая означает, что интерфейс начали показывать,
    #     а не показывать данные.
    ALLOWED_KEYFRAMES = {"ds-rise", "ds-fade", "ds-pop",
                         "ds-slide-in", "ds-shimmer",
                         # появление графиков: кольцо, линия, полоса
                         "ds-arc", "ds-draw", "ds-grow",
                         # индикатор ожидания: вращается только transform
                         # и объявлен infinite, поэтому под потолок не
                         # попадает — как и блик скелета
                         "ds-spin"}
    kf = set(re.findall(r"@keyframes\s+([\w-]+)", css_nc))
    extra = kf - ALLOWED_KEYFRAMES
    check(not extra, "анимации только из списка (%d)" % len(ALLOWED_KEYFRAMES),
          "лишние: " + ", ".join(sorted(extra)))

    # 5б. Потолки длительности. У оболочки — 300 мс: дольше, и
    #     переключение раздела снова читается как подтормаживание.
    #     У появления ГРАФИКА потолок выше (1 с): кольцо и кривая
    #     показывают, как величина сложилась, — это часть чтения
    #     данных, а не украшение перехода. Всё остальное между этими
    #     двумя случаями не попадает.
    CHART_ANIM = ("ds-arc", "ds-draw", "ds-grow")
    long_anim = []
    for m in re.finditer(r"animation\s*:\s*([^;}]+)", css_nc):
        decl = m.group(1)
        if "infinite" in decl:      # блик скелета — индикатор ожидания
            continue
        cap = 1000 if any(k in decl for k in CHART_ANIM) else 300
        for d in re.findall(r"([\d.]+)(ms|s)\b", decl):
            ms = float(d[0]) * (1000 if d[1] == "s" else 1)
            if ms > cap:
                long_anim.append(decl.strip()[:40])
    check(not long_anim, "длительность анимаций в пределах потолков",
          "; ".join(sorted(set(long_anim))))

    # 5в. Анимация НЕ навешивается на :hover. Курсор пролетает над
    #     сотней строк за смену; движение там читается как задержка.
    anim_hover = [m.group(0)[:50] for m in
                  re.finditer(r":hover[^{}]*\{[^}]*?animation\s*:\s*(?!none)", css_nc)]
    check(not anim_hover, "на :hover нет анимаций", "; ".join(anim_hover))

    # 5г. Двигаются только transform и opacity: остальное вызывает
    #     пересчёт раскладки и роняет кадры.
    bad_props = []
    # Тело @keyframes собирается по вложенным скобкам, а не «до первой
    # строки с }»: анимации записаны в одну строку, и жадный шаблон
    # захватывал следом весь остальной файл.
    for m in re.finditer(r"@keyframes\s+[\w-]+\s*\{((?:[^{}]|\{[^{}]*\})*)\}", css_nc):
        for prop in re.findall(r"([a-z-]+)\s*:", m.group(1)):
            # stroke-dashoffset вызывает только перерисовку контура —
            # ни раскладки, ни перекомпоновки слоёв, поэтому он в списке
            # наравне с transform и opacity.
            if prop not in ("transform", "opacity",
                            "background-position", "stroke-dashoffset"):
                bad_props.append(prop)
    check(not bad_props, "в @keyframes двигаются только transform и opacity",
          ", ".join(sorted(set(bad_props))))
