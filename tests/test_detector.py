...
def test_ml_layer_optional():
    ...
    eng = detector.DetectionEngine(use_ml=False)
    # n_real_hits, а не n_regex_hits: правило secret-signature-commit теперь
    # смотрит на НЕ-ЗАГЛУШЕЧНЫЕ совпадения. Прежнее условие
    # (n_regex_hits >= 1 и placeholder_signal == false) выключалось одним
    # словом «TODO» где угодно в файле, потому что placeholder_signal считался
    # по файлу целиком.
    res = eng.process({"actor": "u", "action": "push", "project": "p", "hour": 12,
                       "ts_sim": "2026-06-01T12:00:00",
                       "n_regex_hits": 1, "n_real_hits": 1,
