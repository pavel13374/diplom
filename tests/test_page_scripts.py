...
w.fetch = (u, o) => { calls.push(String(u)); return Promise.resolve({
  ok: true, status: 200,
  json: () => Promise.resolve(fixtureFor(u)),
...
const POLLERS = ['tick', 'refresh', 'pollHealth', 'pollEvents', 'pollRepos',
  'pollInsights', 'pollLogs', 'pollDiag', 'pollOverview', 'pollAttack',
  'pollRules', 'pollWorkflow', 'pollCases', 'pollRisk', 'pollTrends',
  'pollEntities', 'pollIncidents', 'pollAlerts', 'pollScenarios', 'poll'];
(async () => {
