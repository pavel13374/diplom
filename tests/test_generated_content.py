...
def test_ir_report_html():
    ...
    try:
        html = console._incident_report_html(42, inc)
    except Exception as e:
...
SILENT_EXCEPT_LIMIT = 82


def test_silent_except_budget():
    """Сколько отказов в коде остаются невидимыми.

    Из подсчёта исключены две категории, где молчание не является долгом:
    ...
    src = open("console.py", encoding="utf-8").read()
