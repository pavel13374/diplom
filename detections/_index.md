...
Правил: **39**. Покрыто техник модели угроз: **33/46**.
...
**Прямой push в защищённую ветку в обход MR**  
`direct-push-protected` · риск 0.68 · высок.

Изменение внесено прямо в защищённую ветку, минуя merge request и ревью. В нормальном процессе это невозможно: защита ветки на то и стоит. Правило добавлено по результатам трассируемости — техника T1078 исполнялась кампанией review_bypass_push и не ловилась ничем, кроме ночного правила по репозиторию секретов.
...
Средство развёртывания запускает задачу атакующего. Релизы вручную бывают и в норме, поэтому одного признака мало: срабатывает сочетание «ручной запуск + production + защищённая ветка». Требование ночного времени убрано — оно сужало правило до неработоспособности: кампания, прошедшая днём, не детектировалась вовсе (нашёл tests/test_rule_coverage.py).
...
Исключён собственный контент SOC-команды (security_content): правила, гипотезы для охоты, плейбуки и документация ОПИСЫВАЮТ атакующие техники и потому содержат их приметы. Без этого условия правило срабатывало на рабочем продукте аналитика — например на rules/persistence/registry_run_key_modification.yml или hunts/mailbox-forwarding-rule.md.
