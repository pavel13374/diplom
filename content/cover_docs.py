...
def cover_doc(author: str = "", seed_key: str = ""):
    ...
    content = (
        ...
        f"{_para(sec)}\n\n"
        "## Порядок действий\n\n"
        "1. Проверить источник события и полноту полей.\n"
        "2. Сопоставить с текущими правилами детектирования.\n"
        "3. Зафиксировать решение в тикете дежурного.\n"
        "4. При подтверждении — эскалировать по матрице ответственности.\n\n"
        "## Заметки\n\n"
        f"- {body_fix.format(sec=sec)}\n"
        "- открытые вопросы выносим на еженедельный разбор\n"
    )
...
def _para(sec):
    return (f"Раздел описывает, как команда работает с задачей «{sec}». "
            "Ориентируемся на текущие SLA: первичный разбор — 15 минут, "
            "полный разбор инцидента — 4 часа. Если событие не покрыто правилом, "
            "заводим задачу на Detection Engineering и фиксируем пробел в матрице покрытия.")
...
def cover_hotfix(service: str = "collector"):
    ...
    content = (f"# {service} — runtime config\n"
               ...
               "# поднял таймаут: на пике коллектор не успевал подтверждать батч\n")
