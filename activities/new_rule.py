...
class NewRuleActivity:
    ...
    def _mr_description(self, technique: dict) -> str:
        lines = [
            ...
            f"",
            f"### Тестирование",
            f"- [ ] Правило протестировано на семплах логов",
            f"- [ ] False positives проверены",
            f"- [ ] CI pipeline зелёный",
            f"",
            f"### Checklist",
            f"- [x] Sigma-формат валиден",
            f"- [x] UUID уникален",
            f"- [x] MITRE теги добавлены",
            f"- [x] Тестовый семпл добавлен",
        ]
        return "\n".join(lines)
