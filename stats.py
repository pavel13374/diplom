...
#: Точный квантиль стандартного нормального для 95% двустороннего интервала.
#:
#: Раньше в сигнатуре стояла округлённая константа 1.96, и интервалы расходились
#: с эталонными (statsmodels.proportion_confint) в пятом знаке: 0.616979 против
#: 0.616981 на 41/55. На печатное «61.7%» это не влияет, но константа из
#: таблицы — ровно тот случай, когда точное значение достаётся бесплатно:
#: statistics.NormalDist входит в стандартную библиотеку с Python 3.8, и модуль
#: остаётся без внешних зависимостей.
Z95 = NormalDist().inv_cdf(0.975)          # 1.959963984540054


# ----------------------------------------------------------------------
def wilson(successes, total, z=None):
    ...
    z по умолчанию — точный квантиль Z95. Совпадение с statsmodels проверено
    до 1e-12 на 41/55, 0/20, 20/20 и 3/1000 (tests/test_stats.py).
